...
Cursor 对照代码、200 tests 与 `STEP50_DOUBLE_PACK_PASS` 确认 Step 50 已验收。
可写 MAP_SHARED + `RENAME_WHITEOUT`；ABI **v22**；format **v4**。
...
22. Phase 4/内核+控制面 Step 50：`RENAME_DATA` flags 增加 `RENAME_WHITEOUT`；
...
| 13 | `OP_RENAME_DATA` | 从 bounce 读取 old/new name；payload flags 支持 `NOREPLACE` / `EXCHANGE` / `WHITEOUT`；EXCHANGE 与另两者互斥 | 9（v13/v17/v22 扩展 flags） |
...
模块参数、挂载选项、daemon CLI、环境变量、互斥关系和安全默认值的唯一权威表见
[`docs/configuration.md`](docs/configuration.md)。本节只记录已验收的存储组合，不再
复制易漂移的参数清单。
...
| 27 | **远端 cache coherence 仍是最终一致原型** | Step 42/53 保留 durable dirty log、Pub/Sub 提示和 100 ms poll。Step 54 为每个 Redis daemon 注册 3 s TTL session（可配置），每次 mutation Lua 校验精确 session token；过期/冲突返回 ESTALE，writer 不会静默成功。它不是读 lease、fencing generation、range 消息、跨机锁或线性一致协议，读侧最终一致窗口仍在。 | `daemon/src/main.rs`、`daemon/src/meta_redis.rs`、`kestrelfs/cache.c` |
...
| 32 | **可写 mmap 延迟写回仍有无返回值窗口** | Step 50 支持可写 `MAP_SHARED` 与 `mprotect` 合法升级，Step 51 允许共享 folio 保持 dirty；`page_mkwrite` 仍先校验 page-cache epoch。MS_SYNC/fsync/VMA close 会等待 WRITE_DATA，但 `munmap` 无 errno 返回通道，失败依赖 mapping errseq 与后续 fsync/flush 暴露。远端一致性窗口仍含约 100 ms probe 与 worker 调度，尚无 lease。 | `kestrelfs/file.c` |
...
以下命令包含独立 data-dir、模块加载、目标跟随、`readlink`、`ls -l` 与 daemon 重启恢复：

```bash
make -C kestrelfs
(cd daemon && cargo build --release)
sudo insmod kestrelfs/kestrelfs.ko
data_dir=/tmp/kestrelfs-step14-$$
mkdir -p "$data_dir"
./daemon/target/release/kestrelfs-daemon --data-dir "$data_dir" \
  >"$data_dir/daemon.log" 2>&1 &
daemon_pid=$!
...
sudo insmod kestrelfs/kestrelfs.ko
data_dir=/tmp/kestrelfs-step17-$$
mkdir -p "$data_dir"
./daemon/target/release/kestrelfs-daemon --meta "$REDIS_URL" \
  --redis-prefix kestrelfs-step17 --objects s3://kestrelfs-test/step17 \
  --s3-endpoint http://192.168.18.253:9000 --data-dir "$data_dir" \
  >"$data_dir/daemon.log" 2>&1 &
daemon_pid=$!
sleep 2
sudo mkdir -p /mnt/kestrelfs
...
### 7.44 Phase 4/内核+控制面 Step 50 MAP-SHARED-WRITE + WHITEOUT

可写 `MAP_SHARED` 复用 Step 49 的 filemap write-through aops。新增
`page_mkwrite` 在 folio 变脏前检查 page-cache coherence epoch；过期映射先由既有
worker 清 PTE/folio 并重试，失败时返回 `SIGBUS`。`msync`/`fsync` 沿既有
`writepages` → `WRITE_DATA` 路径写回，writable VMA close 也等待映射范围写回；
writeback 失败保留 dirty/error，不把内存状态误报为持久。NVMe read cache 仍在
权威写入前失效。`MAP_PRIVATE` COW 行为保持不变。

`RENAME_DATA` 复用 flags@20 并加入 `RENAME_WHITEOUT=0x4`，不增加 opcode 或
payload 字段。MetaStore 单事务把源移动到目标路径，并在旧路径创建持久
`S_IFCHR` 0:0 inode；FileMetaStore 快照和 Redis v2 Lua revision-CAS 自动覆盖该
状态。目标覆盖继续遵守现有 orphan/GC 语义。`WHITEOUT|NOREPLACE` 按 Linux
语义允许；`EXCHANGE` 与 `WHITEOUT`/`NOREPLACE` 互斥；未知位拒绝。内核要求
`CAP_MKNOD`，lookup/stat 用 `init_special_inode(..., WHITEOUT_DEV)` 恢复 marker。
因为 IPC metadata 首次允许返回 `S_IFCHR`，ABI **v21 → v22**；共享内存、opcode
编号与 cache format **v4** 不变。

Codex 自检：`cargo test` **200 passed**，clippy `-D warnings` 通过，
`make -C kestrelfs` 零警告；真实 Redis 门控测试 **1 passed**。vng guest + loop
输出 `STEP50_MAP_SHARED_WRITE_PASS`、`STEP50_PRIVATE_COW_PASS`、
`STEP50_MSYNC_FSYNC_MUNMAP_PASS`、`STEP50_WHITEOUT_LOOKUP_READDIR_PASS`、
`STEP50_WHITEOUT_FLAGS_PASS`、`STEP50_RESTART_PERSISTENCE_PASS`、
`STEP50_DOUBLE_PACK_PASS`（umount 100 ms）。另回归
`STEP49_CACHE_WRITE_PASS`（181 ms）、更新后的 `STEP46_KERNEL_MMAP_PASS`
（138 ms）、`STEP38_POSIX_EXCHANGE_PASS`（165 ms）与
`STEP33_POSIX_RENAME_PASS`（115 ms）。Step 33/38 过时的 flag 4 拒绝断言已迁为
未知位/非法组合断言。所有 insmod、loop、mount
均只在 vng guest 内执行，daemon 使用独立 data-dir 并保留 daemon.log；未触碰
宿主机 zvol/模块/挂载。详细实现报告见 `docs/remaining-capabilities.md` §9。
Cursor 验收自检（2026-09-20）：200 tests；clippy / make 零警告；复跑 vng
`STEP50_DOUBLE_PACK_PASS`（umount_ms=11）。

### 7.45 Phase 4/内核+文档 Step 51 WRITE-BEHIND + OPS/DOC

普通 buffered write 不再在 `write_iter` 返回前主动等待整个 mapping 写回；folio
标脏后可由内核 BDI flusher、内存压力或显式 durability 路径推进既有
`writepages` → `WRITE_DATA`。`O_SYNC`/`O_DSYNC` 仍经 `generic_write_sync()`；
fsync/fdatasync、MS_SYNC、truncate、close `.flush` 和 writable VMA close 仍等待
相关 dirty folio。VFS 在 `sync_fs(wait=1)` 前已完成 superblock mapping 写回，随后
再发 `OP_SYNC_FS` 后端屏障。失败保留 dirty folio，并通过 mapping/superblock
errseq 在同步点报告；daemon 恢复后可重试。远端 page-cache epoch 清理先撤销 writable
PTE 并写回 dirty folio，成功后才退页；失败时保留 dirty 页，并让 read 或 mmap fault
fail closed。本步没有新增 opcode 或磁盘字段，IPC ABI **v22**、cache format **v4**
均不变。

新增 `docs/configuration.md` 作为模块参数、挂载选项、daemon CLI/环境变量与安全
恢复边界的唯一权威配置表；README/HANDOFF/Phase 4 文档改为链接该表，并清理当前
write-through 等明显过时表述。正常加载仍不自动 wipe 或迁移。

Codex 自检：`STEP51_ASYNC_RETURN_PASS`（daemon 离线时普通 write 0 ms 返回）、
`STEP51_FSYNC_ERROR_PASS`、`STEP51_FSYNC_RECOVERY_PASS`、
`STEP51_SYNCFS_BARRIER_PASS`、`STEP51_DIRTY_COHERENCE_PASS`、
`STEP51_RESTART_READBACK_PASS` 与 `STEP51_WRITE_BEHIND_PASS`（最终 umount 14 ms）。
完整 cargo/clippy/make 与 Step 50/49/46/44
vng 回归结果见 `docs/remaining-capabilities.md` §9。所有 insmod、loop 和 mount
只在 vng guest 内执行；daemon 使用独立 data-dir 并保留 daemon.log。
...
### 7.48 Phase 4/综合 Step 54 OVERNIGHT MEGA（已验收）

RedisMetaStore 为每个 daemon 注册
`<prefix>:meta:v2:sessions:<32-hex-session-id>`，值为同一 session id，默认
TTL 3000 ms、每 TTL/3 心跳续约；`--redis-session-ttl-ms` 可调整且最小 300 ms。
所有 Lua mutation（包括语义失败的 revision 线性化检查）同时校验 session key/token，
缺失或冲突返回 `MetaError::StaleSession` → `ESTALE`，且失效 session 不会被心跳重建。
这只 fence 过期 writer，不是读 lease、fencing generation、分布式锁或线性一致协议。

orphan sweep 不以 daemon TTL 推断内核 fd 已关闭。内核仅在 mount-local
`open_handles==0` 且 last-close `FINALIZE_ORPHAN` 失败时，把 inode 放入模块生命周期
内的 retry list。ABI v23 新增 peek/ack ioctl：daemon 启动及每秒 peek，成功 finalize
（或已经 NotFound）后 ack，再交给既有 durable GC；daemon 在两者之间崩溃会幂等重试。
仍 open 的 inode 不会入队。rmmod 会丢失未处理队列，跨 mount open 引用仍未聚合。

普通动态文件的 fops 新增 `filemap_splice_read` / `iter_file_splice_write`，覆盖
file→pipe、pipe→file 和 sendfile，并沿用 page cache/write-behind/fsync 语义。写回
...
`STEP54_LEASE_HEARTBEAT_NOTIFY_PASS`、`STEP54_LEASE_EXPIRED_FAIL_CLOSED_PASS`、
`STEP54_LEASE_PASS`（Step 53 可见延迟 11 ms）。单 vng+loop 输出
`STEP54_ORPHAN_SWEEP_OPEN_SKIP_PASS`、`STEP54_ORPHAN_SWEEP_RETRY_PASS`、
三项 splice/sendfile PASS、`STEP54_WRITE_PIPE_PRESTAGE_PASS`（2 MiB、512 submissions，
umount 163 ms）。回归 `STEP50_DOUBLE_PACK_PASS`、`STEP51_WRITE_BEHIND_PASS`、
`STEP52_PERF_PASS`；本轮 perf 为 write-behind 915.14 MiB/s、page-cache
2212.31 MiB/s、daemon-free loop hit 175.30 MiB/s / 256 async BIO。所有 insmod、
loop 与 mount 均只在 vng guest。IPC ABI **v23**，cache format **v4** 未变。

Cursor 验收自检（2026-09-21）：203 tests；clippy / make 零警告；Redis 三项 gate PASS；
`STEP54_VFS_PIPE_PASS`（splice/sendfile、orphan open-skip/retry、write_pipe staged_delta=2097152 /
