...
test_id=$$
image=/tmp/kestrel-step46-$test_id.img
mnt=/tmp/mnt-kestrelfs-step46-$test_id
data_dir=/tmp/kestrelfs-step46-$test_id
helper=/tmp/kestrel-step46-mmap-$test_id
loopdev=
daemon_pid=
...
fail() {
	local status=$?
	echo "STEP46_FAIL: line=$1 status=$status"
...
cc -O2 -Wall -Wextra -Werror -std=gnu11 -I kestrelfs \
	tests/test-step46-kernel-mmap.c -o "$helper"
...
if grep -E 'BUG:|KASAN:|use-after-free|general protection fault|hung task' \
	...
	echo 'STEP46_FAIL: kernel diagnostic'
	exit 1
fi
start_ns=$(date +%s%N)
busybox umount "$mnt"
...
trap - EXIT
rm -f "$image" "$helper"
echo "STEP46_MMAP: umount_ms=$umount_ms"
echo STEP46_KERNEL_MMAP_PASS
