...
# operations live exclusively in ./tests/test-step52-dist-node-vng.sh inside each guest.
set -euo pipefail
...
guest_command() {
	...
	printf -v quoted '%q' "$PWD/tests/test-step52-dist-node-vng.sh"
	command+=" $quoted"
	printf '%s' "$command"
}
