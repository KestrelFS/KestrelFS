...
- 当前没有 eviction/LRU、数据 checksum、索引 journal/镜像或 torn-write 检测，
  也没有完整 DMA/零拷贝。
- key 尚未包含 filesystem/MetaStore identity；同一 cache device 必须绑定同一
  metadata namespace。切换 `--data-dir`、Redis prefix 或集群时必须换 cache
  device 或先安全重格式化，否则 inode id 重用可能导致错误命中。
